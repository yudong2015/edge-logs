...
Log Collector image
*/}}
{{- define "edge-logs.logCollector.image" -}}
{{- $registry := .Values.logCollector.image.registry | default .Values.global.imageRegistry -}}
{{- if $registry }}
{{- printf "%s/%s:%s" $registry .Values.logCollector.image.repository .Values.logCollector.image.tag }}
{{- else }}
