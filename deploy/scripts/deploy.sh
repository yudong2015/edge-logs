...
setup_kubectl_context() {
    if [[ -n "${CLUSTER_HOST:-}" ]]; then
        ...
        # Default remote host (backward compatibility)
        log_info "Using default remote host: $REMOTE_HOST"
        KUBECTL_CMD="ssh $REMOTE_HOST kubectl"
    fi
...
apply_manifests() {
    ...
    ssh "${REMOTE_HOST}" "kubectl create namespace $NAMESPACE --dry-run=client -o yaml | kubectl apply -f -"
    ...
    for manifest in "${manifests[@]}"; do
        ...
        # Use envsubst for variable substitution and apply via ssh
        envsubst < "$manifest_file" | \
            ssh "${REMOTE_HOST}" "kubectl apply -f - -n $NAMESPACE"
    done
}
...
wait_for_deployments() {
    ...
    for deployment in "${deployments[@]}"; do
        ...
        if ssh "${REMOTE_HOST}" "kubectl get deployment $deployment -n $NAMESPACE" &> /dev/null; then
            if ! ssh "${REMOTE_HOST}" "kubectl rollout status deployment/$deployment -n $NAMESPACE --timeout=300s"; then
                halt "Pod 启动失败: $deployment"
            fi
        elif ssh "${REMOTE_HOST}" "kubectl get statefulset $deployment -n $NAMESPACE" &> /dev/null; then
            if ! ssh "${REMOTE_HOST}" "kubectl rollout status statefulset/$deployment -n $NAMESPACE --timeout=300s"; then
                halt "Pod 启动失败: $deployment"
            fi
        else
...
verify_deployment() {
    ...
    if ssh "${REMOTE_HOST}" "kubectl exec -n $NAMESPACE deployment/edge-logs-apiserver -- wget -qO- http://localhost:8080/api/v1alpha1/health" &> /dev/null; then
    ...
    if ssh "${REMOTE_HOST}" "kubectl exec -n $NAMESPACE deployment/edge-logs-frontend -- curl -sf http://localhost/healthz" &> /dev/null; then
    ...
    if ssh "${REMOTE_HOST}" "kubectl exec -n $NAMESPACE clickhouse-0 -- wget -qO- http://localhost:8123/ping" &> /dev/null; then
    ...
    if ssh "${REMOTE_HOST}" "kubectl exec -n $NAMESPACE deployment/edge-logs-frontend -- curl -sf http://edge-logs-apiserver.$NAMESPACE.svc.cluster.local:8080/api/v1alpha1/health" &> /dev/null; then
...
show_status() {
    ...
    if [[ "${INGRESS_ENABLED:-false}" == "true" ]]; then
        log_info "Ingress:"
        ssh "${REMOTE_HOST}" "kubectl get ingress -n $NAMESPACE" || log_warn "No ingress found"
        echo ""
    fi
}
